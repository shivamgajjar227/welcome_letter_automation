...
def _get_credential(metadata: RunnerMetadata) -> CredentialRef:
    candidates = [
        metadata.credentials.get(StageName.SUNBIZ_LG.value),
        metadata.credentials.get("sunbiz_lg"),
    ]
...
def _get_base_url(metadata: RunnerMetadata) -> str:
    candidates = [
        metadata.base_urls.get(StageName.SUNBIZ_LG.value),
        metadata.base_urls.get("sunbiz_lg"),
...
def run(driver: WebDriver, metadata: RunnerMetadata) -> StageResult:
    ...
    structured_log(logger, "stage_start", stage=StageName.SUNBIZ_LG.value, task_id=metadata.task_id, url=base_url)

    prsite = PRSiteLG(driver)
    driver.get(base_url)

    try:
        structured_log(logger, "step_start", stage=StageName.SUNBIZ_LG.value, task_id=metadata.task_id, step="login")
        login_artifact = artifacts.capture_screenshot(driver, metadata, StageName.SUNBIZ_LG, "after_login")
        ...
        structured_log(logger, "step_start", stage=StageName.SUNBIZ_LG.value, task_id=metadata.task_id, step="open_board")
        board_artifact = artifacts.capture_screenshot(driver, metadata, StageName.SUNBIZ_LG, "board_loaded")
        ...
        failure_screenshot = artifacts.capture_screenshot(driver, metadata, StageName.SUNBIZ_LG, "failure")
