...
def _get_credential(metadata: RunnerMetadata) -> CredentialRef:
    candidates = [
        metadata.credentials.get(StageName.PR_SITE_LG.value),
        metadata.credentials.get("pr_site_lg"),
    ]
...
def _get_base_url(metadata: RunnerMetadata) -> str:
    candidates = [
        metadata.base_urls.get(StageName.PR_SITE_LG.value),
        metadata.base_urls.get("pr_site_lg"),
...
def run(driver: WebDriver, metadata: RunnerMetadata) -> StageResult:
    ...
    structured_log(logger, "stage_start", stage=StageName.PR_SITE_LG.value, task_id=metadata.task_id, url=base_url)
    ...
    try:
        ...
        failure_screenshot = artifacts.capture_screenshot(driver, metadata, StageName.PR_SITE_LG, "failure")
