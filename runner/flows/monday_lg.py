...
def _get_credential(metadata: RunnerMetadata) -> CredentialRef:
    candidates = [
        metadata.credentials.get(StageName.MONDAY_LG.value),
        metadata.credentials.get("monday_lg"),
    ]
...
def _get_base_url(metadata: RunnerMetadata) -> str:
    candidates = [
        metadata.base_urls.get(StageName.MONDAY_LG.value),
        metadata.base_urls.get("monday_lg"),
...
def run(driver: WebDriver, metadata: RunnerMetadata) -> StageResult:
    ...
    structured_log(logger, "stage_start", stage=StageName.MONDAY_LG.value, task_id=metadata.task_id, url=base_url)

    monday = MondayLGPage(driver)
    driver.get(base_url)

    try:
        structured_log(logger, "step_start", stage=StageName.MONDAY_LG.value, task_id=metadata.task_id, step="login")
        if monday.is_login_page():
            monday.login(cred.username, cred.password)
        else:
            structured_log(
                logger,
                "login_skipped",
        ...
        login_artifact = artifacts.capture_screenshot(driver, metadata, StageName.MONDAY_LG, "after_login")
        ...
        structured_log(logger, "step_start", stage=StageName.MONDAY_LG.value, task_id=metadata.task_id, step="open_board")
        monday.click_welcome_letter_qc()
        board_artifact = artifacts.capture_screenshot(driver, metadata, StageName.MONDAY_LG, "board_loaded")
        ...
        failure_screenshot = artifacts.capture_screenshot(driver, metadata, StageName.MONDAY_LG, "failure")
